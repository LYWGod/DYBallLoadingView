...
typedef NS_ENUM(NSUInteger, BallMoveDirection) {
    BallMoveDirectionPositive = 1,    // 正向运动
    BallMoveDirectionNegative = -1    // 逆向运动
};
...
@property (nonatomic, assign) BallMoveDirection   moveDirection;
...
+ (instancetype)loadingViewInView:(UIView *)view {
    BallLoadingView *loadingView = [[BallLoadingView alloc] initWithFrame:view.bounds];
...
- (void)setupUI {
    ...
    self.greenBall.backgroundColor = ColorRGB(35, 246, 235);
    ...
    self.redBall.backgroundColor = ColorRGB(255, 46, 86);
    ...
    self.blackBall.backgroundColor = ColorRGB(12, 11, 17);
    ...
    // 方向
    self.moveDirection = BallMoveDirectionPositive;
...
- (void)updateBallAnimations {
    if (self.moveDirection == BallMoveDirectionPositive) { // 正向
        CGPoint center = self.greenBall.center;
        center.x += kBallSpeed;
        self.greenBall.center = center;
        ...
        if (CGRectGetMaxX(self.greenBall.frame) >= self.containerView.bounds.size.width || CGRectGetMinX(self.redBall.frame) <= 0) {
            // 切换为反向
            self.moveDirection = BallMoveDirectionNegative;
    ...
    }else if (self.moveDirection == BallMoveDirectionNegative) { // 反向
        // 更新绿球位置
        CGPoint center = self.greenBall.center;
        center.x -= kBallSpeed;
        self.greenBall.center = center;
